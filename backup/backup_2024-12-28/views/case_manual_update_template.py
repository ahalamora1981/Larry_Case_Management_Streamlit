...
def case_update_staff_page(user_type: str = None):
    ...
    with col_22:
        if not multi_selection:  # 单选
            ...
            with col_222:
                if case_selected is None or case_selected.repayment_plan is None:
                    repayment_plan_value = ""
                else:
                    repayment_plan_value = case_selected.repayment_plan
                    
                repayment_plan = st.text_area( 
                    "还款计划",
                    value=repayment_plan_value,
                    height=200,
                )
                
                if case_selected is None or case_selected.remark is None:
                    remark_value = ""
                else:
                    remark_value = case_selected.remark
